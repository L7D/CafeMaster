...
namespace CafeMaster_UI.Lib
{
	...
	static class Utility
	{
		...
		[CallerLineNumber] int debugTraceCallerLine = -1 )
		{
			try
			{
				...
				File.AppendAllText( path + @"\error.log",
					string.Format( "MilkColoredCafeStaff_ErrorLOG_{0} @{1} :	{2} -> #{3}		{4}:{5}" + Environment.NewLine,
		...
		private static readonly string[ ] sizeString = { "bytes", "KB", "MB", "GB", "TB", "PB", "EB", "ZB", "YB" };
		...
		public static string SizeSuffix( Int64 value, int decimalPlaces = 1 )
		{
			...
			return string.Format( "{0:n" + decimalPlaces + "} {1}", dValue, sizeString[ i ] );
		}
		...
		public static string GetMD5Hash( string fileName )
		{
			...
			using ( FileStream stream = File.OpenRead( fileName ) )
			{
				...
				byte[ ] resultArr = ( new MD5CryptoServiceProvider( ) ).ComputeHash( stream );
		...
		public static bool ProgramValidCheck( )
		{
			try
			{
				HttpWebRequest request = ( HttpWebRequest ) WebRequest.Create( "http://textuploader.com/d99ug/raw" );
				request.Method = "GET";
				...
				using ( HttpWebResponse response = ( HttpWebResponse ) request.GetResponse( ) )
				{
					using ( Stream responseStream = response.GetResponseStream( ) )
					{
						using ( StreamReader reader = new StreamReader( responseStream ) )
						{
							...
							updateInformation.LoadXml( xmlData );

							foreach ( XmlNode node in updateInformation.DocumentElement.ChildNodes )
							{
								if ( node.Name == "V" + GlobalVar.CURRENT_VERSION )
								{
									if ( string.Equals( Utility.GetMD5Hash( Application.ExecutablePath ), node.InnerText ) )
										return true;
								}
							}
			...
			{
				Utility.WriteErrorLog( ex.Message, LogSeverity.EXCEPTION );
				return false;
			}
		}
		...
		// L7D(smhjyh2007) -> smhjyh2007
		public static string GetNaverIDFromNicknameString( string nickName )
		{
			try
			{
				int start = nickName.IndexOf( '(' );
				return nickName.Substring( start + 1, nickName.Length - start - 2 );
			}
			catch
			{
				return "NULL";
		...
		public static void ProgramErrorHappendTick( )
		{
			Main mainForm = GetMainForm( );

			if ( mainForm != null )
			{
				if ( mainForm.UPDATE_AVAILABLE.Visible )
				{
					//513, 16
					mainForm.CURRENT_PROGRESS_LABEL.Location = new Point( 520, 13 );
					mainForm.PROGRAM_ERROR_HAPPEND.Location = new Point( 830, 10 );
				}
				else
				{
					mainForm.CURRENT_PROGRESS_LABEL.Location = new Point( 548, 13 );
					mainForm.PROGRAM_ERROR_HAPPEND.Location = new Point( 864, 10 );
				}
					

				mainForm.PROGRAM_ERROR_HAPPEND.Visible = true;
				mainForm.ERROR_ICON_HIGHLIGHT_TIMER.Start( );
			}
		}
	}
