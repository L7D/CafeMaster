...
namespace CafeMaster_UI.Lib
{
	static class Config
	{
		...
		{
			string configValue = "# 우윳빛깔 카페스탭 설정 파일 ver " + GlobalVar.CURRENT_VERSION + " #$# 이 파일을 함부로 수정하지 마세요! #$$SoundEnable -> 1$SyncInterval -> 30$CaptureEnable -> 1$UXSendEnable -> 0$ThemeEnable -> 1$AutoLoginRecommendNeed -> 1".Replace( "$", Environment.NewLine );
